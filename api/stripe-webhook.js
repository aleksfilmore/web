...
export const config = {
  api: {
    bodyParser: false, // CRITICAL: Disable body parsing for raw body access
  },
};
  ...
  const databaseUrl = process.env.DATABASE_URL;
    ...
    if (client || db) {
      try {
        ...
        const paramsSelect = [orderId];
        console.log('🔍 Checking existing order:', orderId);

        // Check if order exists
        const selRes = await runQuery(client, db, selectSql, paramsSelect);
        console.log('📋 Query result:', selRes);

        const orderRows = normalizeRows(selRes);
        console.log('📊 Normalized rows:', orderRows);
        ...
        if (existingOrder) {
          ...
          const paramsUpdate = ['completed', JSON.stringify({ 
          ...
          // If this was a duplicate somehow, update it
          if (existingOrder && existingOrder.status !== 'completed') {
            const paramsUpdate2 = ['completed', JSON.stringify({ 
              stripe_session: session,
              product_name: productName,
              duplicate_resolved: new Date().toISOString()
            }), orderId];
            
            await runQuery(client, db, updateSql, paramsUpdate2);
            console.log('✅ Duplicate order resolved');
          }
        }
      } catch (dbError) {
...
function generateAccessToken(email, sessionId) {
  // Standardized token format: base64(email:sessionId:timestamp)
